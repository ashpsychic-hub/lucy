    ...
    <>
      ...
      <div style={{ opacity:v?1:0, transition:"opacity .5s", paddingBottom: isMob?100:80 }}>
        ...
        <div style={{ padding: isMob?"0 16px":"0 40px", maxWidth:1000, margin:"0 auto" }}>
          ...
          <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize: isMob?"clamp(28px,8vw,40px)":"clamp(36px,4.5vw,56px)", fontWeight:700, lineHeight:1.0, marginBottom:10 }}>{movie.title}</h1>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16, flexWrap:"wrap" }}>
            <span style={{ fontSize: isMob?12:13, color:G.muted, letterSpacing:".04em", textTransform:"uppercase" }}>A film by</span>
            <span style={{ fontSize: isMob?13:15, fontWeight:600, color:G.lavender, fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic" }}>{movie.director||movie.uploader||"Unknown"}</span>
            <span style={{ color:G.dim }}>·</span>
            <span style={{ fontSize:12, color:G.muted }}>{movie.year}</span>
            {movie.duration&&<><span style={{ color:G.dim }}>·</span><span style={{ fontSize:12, color:G.muted }}>{movie.duration}</span></>}
          </div>
          <div style={{ display:"flex", gap:10, marginBottom:32, flexWrap:"wrap" }}>
            <button className="btn-primary" style={{ display:"flex", alignItems:"center", gap:10, padding: isMob?"12px 24px":"14px 32px", fontSize:14 }} onClick={()=>{ setActiveEp(null); setPlayerOpen(true); }}>
              <svg width="12" height="14" viewBox="0 0 12 14" fill="none"><path d="M1 1l10 6L1 13V1z" fill="white"/></svg>
              Watch Film
            </button>
            <button className="btn-ghost" style={{ padding: isMob?"11px 18px":"13px 22px", fontSize:13 }}>+ Watchlist</button>
          </div>
          <div style={{ marginBottom:40, maxWidth:680 }}>
            <p style={{ color:G.textSoft, lineHeight:1.95, fontSize: isMob?13:15, borderLeft:"2px solid #9b6dff44", paddingLeft:20 }}>{movie.desc||"No synopsis provided."}</p>
          </div>
          <div style={{ display:"flex", gap:0, borderBottom:"1px solid #221d35", marginBottom:28 }}>
            {[{id:"story",label:"Story"},{id:"process",label:"Director's Notes"},{id:"credits",label:"Credits"},...(episodes.length>1?[{id:"episodes",label:"Episodes"}]:[])].map(t=>(
              <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{ padding: isMob?"10px 12px":"11px 20px", fontSize: isMob?11:12, fontWeight: activeTab===t.id?600:400, letterSpacing:".04em", textTransform:"uppercase", color: activeTab===t.id?"#9b6dff":"#6b6585", background:"none", borderBottom: activeTab===t.id?"2px solid #9b6dff":"2px solid transparent", marginBottom:-1, transition:"color .2s", whiteSpace:"nowrap" }}>{t.label}</button>
            ))}
          </div>
          <div style={{ display: isMob?"flex":"grid", flexDirection: isMob?"column":undefined, gridTemplateColumns: isMob?undefined:"1fr 280px", gap: isMob?24:40 }}>
            <div>
              {activeTab==="story" && (
                <div>
                  <p style={{ color:"#ccc8e8", lineHeight:1.9, fontSize: isMob?13:14, marginBottom:24 }}>{movie.desc||"No synopsis available."}</p>
                  <div style={{ background:"#0e0c1a", border:"1px solid #221d35", borderRadius:12, padding: isMob?"14px 16px":"18px 22px", marginBottom:28 }}>
                    <div style={{ fontSize:10, color:"#6b6585", letterSpacing:".1em", textTransform:"uppercase", marginBottom:12, fontWeight:600 }}>Production Disclosure</div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: isMob?10:14 }}>
                      {[{label:"AI Tools",value:movie.aiTool||movie.ai_tool||"Not specified"},{label:"AI Involvement",value:movie.aiType||"Not specified"},{label:"Director",value:movie.director||"Not specified"},{label:"Studio",value:movie.studio||"Independent"}].map(({label,value})=>(
                        <div key={label}>
                          <div style={{ fontSize:10, color:"#6b6585", letterSpacing:".06em", textTransform:"uppercase", marginBottom:3 }}>{label}</div>
                          <div style={{ fontSize:12, fontWeight:500, color:"#f0eeff" }}>{value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div style={{ borderRadius:12, overflow:"hidden", aspectRatio:"2.39/1", background:"#120f1f" }}>
              ...
              {activeTab==="process" && (
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
                    <div style={{ width:44, height:44, borderRadius:"50%", background:"linear-gradient(135deg,#9b6dff44,#5c3d9944)", border:"1px solid #9b6dff33", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Cormorant Garamond',serif", fontWeight:700, color:"#9b6dff", fontSize:18 }}>{(movie.director||movie.uploader||"?").charAt(0)}</div>
                    <div>
                      <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:16, fontWeight:600, color:"#f0eeff" }}>{movie.director||movie.uploader||"Director"}</div>
                      <div style={{ fontSize:11, color:"#6b6585" }}>Director's Statement</div>
                    </div>
                  </div>
                  <div style={{ background:"#0e0c1a", border:"1px solid #221d35", borderRadius:12, padding: isMob?"16px":"22px", marginBottom:24 }}>
                    {movie.directorNotes?(
                      <p style={{ color:"#ccc8e8", lineHeight:1.95, fontSize: isMob?13:14, fontStyle:"italic" }}>"{movie.directorNotes}"</p>
                    ):(
                      <div style={{ textAlign:"center", padding:"24px 0" }}>
                        <div style={{ fontSize:28, marginBottom:8, opacity:.3 }}>✍</div>
                        <div style={{ fontSize:13, color:"#6b6585" }}>Director has not added a statement yet.</div>
                      </div>
                    )}
                  </div>
                  <div style={{ background:"#0e0c1a", border:"1px solid #221d35", borderRadius:12, padding: isMob?"14px 16px":"18px 22px" }}>
                    <div style={{ fontSize:10, color:"#6b6585", letterSpacing:".1em", textTransform:"uppercase", marginBottom:16, fontWeight:600 }}>Creation Process</div>
                    {[{label:"Primary AI Tool",value:movie.aiTool||movie.ai_tool||"Not disclosed"},{label:"Human Involvement",value:movie.aiType||"Not specified"},{label:"Original Score",value:"Not specified"}].map(({label,value})=>(
                      <div key={label} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid #221d35" }}>
                        <span style={{ fontSize:12, color:"#6b6585" }}>{label}</span>
                        <span style={{ fontSize:12, fontWeight:500, color:"#f0eeff" }}>{value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab==="credits" && (
                <div>
                  <div style={{ background:"#3ecfb211", border:"1px solid #3ecfb233", borderRadius:12, padding: isMob?"12px 16px":"16px 20px", marginBottom:24, display:"flex", gap:12 }}>
                    <div style={{ fontSize:18 }}>✓</div>
                    <div>
                      <div style={{ fontSize:12, fontWeight:600, color:"#3ecfb2", marginBottom:4 }}>Originality Declared</div>
                      <div style={{ fontSize:11, color:"#6b6585", lineHeight:1.6 }}>This creator has declared this work is original and AI tools have been accurately disclosed.</div>
                    </div>
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {[{role:"Director",name:movie.director||"Unknown"},{role:"Screenwriter",name:movie.writer||movie.director||"Unknown"},{role:"Visual Effects",name:movie.aiTool||"AI-Generated"},{role:"Production",name:movie.studio||"Independent"}].map((c,i)=>(
                      <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:"#120f1f", borderRadius:10, border:"1px solid #221d35" }}>
                        <div style={{ width:34, height:34, borderRadius:"50%", background:"linear-gradient(135deg,#9b6dff33,#5c3d9933)", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, color:"#9b6dff", fontSize:12, flexShrink:0 }}>{c.name.charAt(0)}</div>
                        <div>
                          <div style={{ fontSize:13, fontWeight:600, color:"#f0eeff" }}>{c.name}</div>
                          <div style={{ fontSize:10, color:"#6b6585", textTransform:"uppercase", letterSpacing:".06em" }}>{c.role}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeTab==="episodes" && (
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {episodes.map(ep=>(
                    <div key={ep.id} onClick={()=>{ setActiveEp(ep); setPlayerOpen(true); }} style={{ display:"flex", gap:14, background:"#120f1f", borderRadius:12, padding: isMob?"10px":"14px", border:"1px solid #221d35", cursor:"pointer" }}>
                      <div style={{ position:"relative", flexShrink:0, width: isMob?110:150, height: isMob?62:84, borderRadius:8, overflow:"hidden" }}>
                        ...
                        <div style={{ position:"absolute", inset:0, background:"#0005", display:"flex", alignItems:"center", justifyContent:"center" }}>
                          <div style={{ width:32, height:32, borderRadius:"50%", background:"#9b6dffcc", display:"flex", alignItems:"center", justifyContent:"center" }}>
                            <svg width="11" height="13" viewBox="0 0 11 13" fill="none"><path d="M1 1l9 5.5L1 12V1z" fill="white"/></svg>
                          </div>
                        </div>
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontWeight:600, fontSize: isMob?12:14, color:"#f0eeff", marginBottom:5 }}>{ep.title}</div>
                        <p style={{ fontSize: isMob?11:12, color:"#6b6585", lineHeight:1.6 }}>{ep.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div>
              <div style={{ background:"#120f1f", border:"1px solid #221d35", borderRadius:12, padding:"16px", marginBottom:20 }}>
                <div style={{ fontSize:10, fontWeight:600, color:"#6b6585", letterSpacing:".1em", textTransform:"uppercase", marginBottom:14 }}>Film Details</div>
                {[{label:"Director",value:movie.director},{label:"Year",value:movie.year},{label:"Duration",value:movie.duration},{label:"Category",value:movie.category},{label:"AI Tool",value:movie.aiTool||movie.ai_tool},{label:"Views",value:movie.views||"0"}].filter(r=>r.value).map(({label,value})=>(
                  <div key={label} style={{ display:"flex", justifyContent:"space-between", padding:"7px 0", borderBottom:"1px solid #221d35" }}>
                    <span style={{ fontSize:11, color:"#6b6585" }}>{label}</span>
                    <span style={{ fontSize:11, fontWeight:500, color:"#f0eeff" }}>{value}</span>
                  </div>
                ))}
              </div>
              {related.length>0 && (
                <div>
                  <div style={{ fontSize:10, fontWeight:600, color:"#6b6585", letterSpacing:".1em", textTransform:"uppercase", marginBottom:14 }}>More Like This</div>
                  <div style={{ display:"flex", flexDirection: isMob?"row":"column", gap:10, overflowX: isMob?"auto":"visible", scrollbarWidth:"none" }}>
                    {related.map(m=>(
                      <div key={m.id} onClick={()=>nav("watch",m)} style={{ flexShrink:0, minWidth: isMob?130:"auto", background:"#120f1f", borderRadius:10, overflow:"hidden", border:"1px solid #221d35", cursor:"pointer" }}>
                        <img src={m.thumb} alt={m.title} style={{ width:"100%", height: isMob?74:60, objectFit:"cover" }}/>
                        <div style={{ padding:"8px 10px 10px" }}>
                          <div style={{ fontWeight:600, fontSize:11, color:"#f0eeff", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.title}</div>
                          <div style={{ fontSize:10, color:"#6b6585", marginTop:2, display:"flex", justifyContent:"space-between" }}><span>{m.year}</span><RatingDot score={m.rating}/></div>
                        </div>
                      </div>
                    ))}
                  </div>
